  ...
  return (
    <div className="text-center p-4 mt-4 text-purple">
      ...
      <div className=" flex justify-center gap-2">
        <button className="shadow-custom rounded-full grow h-12" type="button" style={inProgress ? { border: "2px solid var(--purple)" } : { border: "" }} onClick={() => { setCurrentGoal(null); setInProgress(true); setSubmitError(""); }}>In Progress Goals</button>
        <button className="shadow-custom rounded-full grow h-12" type="button" style={!inProgress ? { border: "2px solid var(--purple)" } : { border: "" }} onClick={() => { setCurrentGoal(null); setInProgress(false); setSubmitError(""); }}>Completed Goals</button>
      </div>
      <div>
